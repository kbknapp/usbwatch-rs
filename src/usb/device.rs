...
impl<'a> From<&'a tokio_udev::Device> for UsbDevice {
    fn from(d: &tokio_udev::Device) -> Self {
        Self {
            ID_MODEL: d.property_value("ID_MODEL").map(|v| v.to_string_lossy().to_string()),
            ID_MODEL_ENC: d.property_value("ID_MODEL_ENC").map(|v| v.to_string_lossy().to_string()),
            ID_MODEL_FROM_DATABASE: d.property_value("ID_MODEL_FROM_DATABASE").map(|v| v.to_string_lossy().to_string()),
            ID_MODEL_ID: d.property_value("ID_MODEL_ID").map(|v| v.to_string_lossy().to_string()),
            ID_SERIAL: d.property_value("ID_SERIAL").map(|v| v.to_string_lossy().to_string()),
            ID_SERIAL_SHORT: d.property_value("ID_SERIAL_SHORT").map(|v| v.to_string_lossy().to_string()),
            ID_VENDOR: d.property_value("ID_VENDOR").map(|v| v.to_string_lossy().to_string()),
            ID_VENDOR_ENC: d.property_value("ID_VENDOR_ENC").map(|v| v.to_string_lossy().to_string()),
            ID_VENDOR_FROM_DATABASE: d.property_value("ID_VENDOR_FROM_DATABASE").map(|v| v.to_string_lossy().to_string()),
            ID_VENDOR_ID: d.property_value("ID_VENDOR_ID").map(|v| v.to_string_lossy().to_string()),
            PRODUCT: d.property_value("PRODUCT").map(|v| v.to_string_lossy().to_string()),
            ..Default::default()
        }
    }
}
